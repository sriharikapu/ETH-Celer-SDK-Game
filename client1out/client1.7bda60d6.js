...
const client = new celer.Client('http://localhost:29979'); //client 1 address: 0xeE87af530753DE52088b5D60325e0ef24C3357C9
...
(async function () {
  ...
  const opponentAddress = '0x05E4664a7459972EeD278cee62d8439Ba9EEDAbA'; //openEthChannel(amountWei: string, peerAmountWei: string): Promise<string>
  ...
  const stateValidator = function stateValidator(state) {
    ...
    console.log('transactionNo: ', transactionNo); // client.sendState(sessionID, opponentAddress, state);

    return true;
  };

  sessionID = await client.createAppSession(appInfo, stateValidator);
  console.log('sessionID: ', sessionID); //only client1 code below:
  // await timeout(5000);
...
})().catch(console.log); //have two copies of client so can run locally
